...
function Login() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        // Update user status = true when logged in
        ...
        // ✅ Set up auto-offline status on page unload
  ...
  // ✅ Optional logout handler (if you want to add a logout button later)
  const logout = async () => {
    const user = auth.currentUser;
    if (user) {
      await updateDoc(doc(db, 'USERS', user.uid), { status: false });
    }
    await signOut(auth);
  };
    ...
    <div className="login-container">
      <div className="form">
        ...
        <span className="px-3 text-gray-700">
          Currently only <strong>Google Sign-In</strong> is working!
        </span>
      </div>
    </div>
  );
